...
def medir_diferencia_vectorizacion(n=20_000_000):
    """
...
def calcular_iva_todos(valores):
    """
    Calcula el IVA (19 %) para todos los valores declarados.
    """
    valores = np.array(valores)
    return valores * 0.19

def calcular_valor_con_iva(valores):
    """
    Retorna los valores con el IVA incluido.
    """
    valores = np.array(valores)
    return valores * 1.19

def redondear_a_miles(valores):
    """
    Redondea los valores al millar más cercano.
    """
    valores = np.array(valores)
    return np.round(valores, -3)
